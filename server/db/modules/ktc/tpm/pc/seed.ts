/**
 * ktc/tpm/pc 模块种子数据（人员管控）
 */
import type { Database } from "sql.js"
import { faker } from "@faker-js/faker/locale/zh_CN"

/** 表为空时插入默认数据（服务启动时调用） */
export function seedPcDefaults(_db: Database) {
  // pc 模块没有需要初始化的默认数据
}

/** 生成 Faker 大数据（npm run seed 时调用） */
export function seedPcFakeData(db: Database) {
  const categories = [
  ...
  // 生成树结构
  console.log("🌱 正在生成 pc 树结构...")
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["yhgl", "企业管理", 0, 1, "office-building"])
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["yhgl", "企业管理-已标记", 0, 2, "star"])
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["yhgl", "企业管理-回收站", 0, 3, "delete"])
  const yhglRootId = db.exec("SELECT last_insert_rowid()")[0]?.values[0]?.[0] as number
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["yhgl", "国有企业", yhglRootId, 1, "bank"])
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["yhgl", "民营企业", yhglRootId, 2, "factory"])
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["yhgl", "外资企业", yhglRootId, 3, "globe"])

  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["gxjg", "全部机构", 0, 1, "building"])
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["gxjg", "合作机构", 0, 2, "handshake"])
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["gxjg", "关联机构", 0, 3, "link"])
  const gxjgRootId = db.exec("SELECT last_insert_rowid()")[0]?.values[0]?.[0] as number
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["gxjg", "总公司", gxjgRootId, 1, "building"])
  db.run("INSERT INTO ktc_pc_trees (type, name, parent_id, sort_order, icon) VALUES (?, ?, ?, ?, ?)", ["gxjg", "分公司", gxjgRootId, 2, "office"])
  console.log("  ✅ 已生成树结构数据")

  // 生成类别
  console.log("🌱 正在生成 pc 类别数据...")
  const categoryCount = db.exec("SELECT COUNT(*) FROM ktc_pc_categories")
  if (categoryCount[0]?.values[0]?.[0] === 0) {
    for (const cat of categories) {
      db.run("INSERT INTO ktc_pc_categories (name, code, description, sort_order) VALUES (?, ?, ?, ?)", [cat.name, cat.code, cat.description, cat.sort_order])
    }
    console.log(`  ✅ 已生成 ${categories.length} 条类别数据`)
  } else {
    console.log("  ⏭️ 类别数据已存在，跳过生成")
  }

  // 生成人员数据
  console.log("🌱 正在生成 pc 人员数据...")
  const treeNodeRows = db.exec("SELECT id, type FROM ktc_pc_trees WHERE status = 1")
