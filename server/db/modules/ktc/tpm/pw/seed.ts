/**
 * ktc/tpm/pw 模块种子数据（人员预警）
 */
import type { Database } from "sql.js"

/** 表为空时插入默认数据（服务启动时调用） */
export function seedPwDefaults(db: Database) {
  const result = db.exec("SELECT COUNT(*) FROM ktc_pw_persons")
...
export function seedPwFakeData(_db: Database) {
  // pw 模块使用默认示例数据即可
}
