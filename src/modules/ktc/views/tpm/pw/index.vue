	...
	<div class="pw-container">
		...
		<div class="data-section">
			...
			<div class="action-bar">
				...
				<HxImporter
					ref="importerRef"
					:upload-action="'/ktc/tpm/pw/import'"
					:method="'post'"
					template-file-name="人员预警导入模板.xlsx"
					:template-url="pw.getImportTemplate()"
					:max-size="10"
					button-text="导入"
				...
				/>
					<HxExporter
						ref="exporterRef"
						:export-action="'/ktc/tpm/pw/export'"
						:method="'post'"
...
import { pw } from "@/modules/ktc/api"
import type { ListItem } from "@/modules/ktc/api/tpm/pw/types"
...
const formColumns = computed<FormField[]>(() => [
	{
		...
		placeholder: "请选择业务类别",
		remote: {
			url: "/ktc/tpm/pw/dict/business-category",
			labelKey: "label",
			valueKey: "value"
		},
		colSpan: 1
	},
	{
		...
		placeholder: "请选择人员分值",
		remote: {
			url: "/ktc/tpm/pw/dict/person-score",
			labelKey: "label",
			valueKey: "value"
		},
		colSpan: 1
	},
	{
		...
		placeholder: "请选择预警类型",
		remote: {
			url: "/ktc/tpm/pw/dict/warning-type",
			labelKey: "label",
			valueKey: "value"
		},
		colSpan: 1
	},
	{
		...
		placeholder: "请选择数据来源",
		remote: {
			url: "/ktc/tpm/pw/dict/data-source",
			labelKey: "label",
			valueKey: "value"
		},
		colSpan: 1
	},
	{
		...
		placeholder: "请选择管理类别",
		remote: {
			url: "/ktc/tpm/pw/dict/manage-category",
			labelKey: "label",
			valueKey: "value"
		},
		colSpan: 1
	},
	{
		...
		placeholder: "请选择户籍地址",
		remote: {
			url: "/ktc/tpm/pw/dict/household-address",
			labelKey: "label",
			valueKey: "value"
		},
		colSpan: 1
	...
	},
	{
		...
		placeholder: "请选择管理部门",
		remote: {
			url: "/ktc/tpm/pw/dict/manage-dept",
			labelKey: "label",
			valueKey: "value"
		},
		colSpan: 1
...
const loadTableData = async () => {
	...
	try {
		...
		const res = await pw.getList(query)
...
const handleAddSubmit = async () => {
	try {
		...
		if (dialogMode.value === "add") {
			res = await pw.create(submitData)
		} else {
			res = await pw.update({ id: editingRow.value!.id, ...submitData })
		}
...
const handleDelete = (row: ListItem) => {
	ElMessageBox.confirm(`确认删除 "${row.name}" 吗？`, "提示", {
		...
		.then(async () => {
			try {
				const res = await pw.delete(row.id)
