	...
	<div class="pc-container">
		...
		<div class="right-panel">
			...
			<div class="data-section">
				...
				<HxImporter
					ref="importerRef"
					:upload-action="'/ktc/tpm/pc/import'"
					:method="'post'"
					template-file-name="人员信息导入模板.xlsx"
					:template-url="'/ktc/tpm/pc/template'"
					:max-size="10"
					button-text="导入"
					dialog-title="导入人员信息"
				...
				/>
						<HxExporter
							ref="exporterRef"
							:export-action="'/ktc/tpm/pc/export'"
							:method="'post'"
...
import { pc, type TreeNode, type ListItem, type DictItem, FOLLOW_LEVEL_OPTIONS } from "@/modules/ktc/api"
...
const loadTreeData = async (type: string) => {
	...
	try {
		let res
		if (type === "yhgl") {
			res = await pc.getYhglTree()
		} else {
			res = await pc.getGxjgTree()
		}
...
const loadTableData = async () => {
	...
	try {
		...
		const res = await pc.getList(query)
...
const handleAddSubmit = async () => {
	try {
		...
		if (dialogMode.value === "add") {
			res = await pc.create(submitData)
		} else {
			res = await pc.update({
...
const handleDelete = (row: ListItem) => {
	ElMessageBox.confirm(`确认删除 "${row.name}" 吗？`, "提示", {
		...
		.then(async () => {
			try {
				const res = await pc.delete(row.id)
...
const handleCategorySubmit = async () => {
	...
	try {
		...
		const res = await pc.assignCategory({
...
const handleFollowSubmit = async () => {
	...
	try {
		...
		const res = await pc.updateStatus({
...
const loadDataSourceOptions = async () => {
	try {
		const res = await pc.getSourceOptions()
...
const loadCategoryOptions = async () => {
	try {
		const res = await pc.getCategoryOptions()
...
const loadAddressOptions = async () => {
	try {
		const res = await pc.getAddressOptions()
